...
function renderRecipeList(listOverride) {
  ...
  if (!list.length) {
    ...
    div.textContent = "目前還沒有任何食譜，可以先用下方「新增食譜」新增一筆。";
  ...
  sorted.forEach((recipe) => {
    ...
    titleEl.textContent = recipe.name || recipe.title || "未命名食譜";
    ...
    metaEl.textContent = serving ? `${category} · 份量：${serving}` : category;
    ...
    function renderPreview() {
      ...
      ingLabel.textContent =
        ratioVal === 1 ? "材料（原始配方）：" : `材料（x${ratioVal} 倍後）：`;
    ...
    const labelParts = [];
    if (recipe.sourceType === "personal") labelParts.push("自家配方");
    if (recipe.sourceType === "pro") labelParts.push("專業配方");
    if (recipe.sourceType === "book") labelParts.push("書籍 / 出版品");
    if (recipe.proAuthor) labelParts.push(recipe.proAuthor);

    if (labelParts.length) {
      tags.appendChild(document.createTextNode(labelParts.join(" · ")));
    }
    ...
    pinBtn.textContent = recipe.pinned ? "取消釘選" : "釘選";
